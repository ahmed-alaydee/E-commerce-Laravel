...
@section('content')
    ...
    <section class="breadcrumb-section section-b-space" style="padding-top:20px;padding-bottom:20px;">
     ...
     @if ($cartItems->count() > 0)
            ...
            <a href="{{ route('shop.index') }}" class="btn btn-primary mt-3 mb-4">Shop Now </a>
            </div>
            </div>
     @endif
                    ...
                    <table class="table cart-table">
                       ...
                       @foreach ($cartItems as $item )
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('shop.product.details',['slug'=>$item->model->slug]) }}">A Porro
                                    ...
                                    <div class="qty-box">
                                        <div class="input-group">
                                            <input type="number" name="quantity"
                                                data-rowid="{{$item->rowId}}" onchange="updateQuantity(this)" class="form-control input-number" value="{{ $item->qty }}">
                                        </div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <a href="javascript:void(0)" onclick="removeFromCart('{{ $item->rowId }}')" >
                                        <i class="fas fa-times"></i>
                                    </a>
                                </td>
                            </tr>
                                ...
                                <div class="cart-box-details">
                                    <div class="total-details">
                                        <div class="top-details">
                                            ...
                                            <h6>Tax <span>{{ Cart::instance('cart')->total()}}</span></h6>

                                            <h6>Total <span>{{ Cart::instance('cart')->Tax()}}</span></h6>
                                        </div>
...
<form id="updateCategory" action="{{ route('cart.update') }}" method="post">
@csrf
@method('put')
...
@push('js')
<script>
    {{--  الاسكربت ده مسول ان لما ادوس علي زرار التنزيل والصعود يزود فعلا المنتج ويزود السعر   --}}
